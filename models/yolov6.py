...
class Head_layers(nn.Module):
    def __init__(self,in_channels,reg_max = 16,num_classes = 3, num_anchors = 3):
        ...
        # reg_pred0
        self.reg_pred = nn.Conv2d(in_channels=in_channels, out_channels=4 * num_anchors, kernel_size=1)

        self.object = nn.Conv2d(in_channels=in_channels, out_channels=1 * num_anchors, kernel_size=1)
    ...
    def forward(self,x):
        ...
        obj_output = self.object(reg_feat)

        return torch.cat([cls_output,reg_output,obj_output],dim=1)
...
class CSPNeXtBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = Conv_v5(in_channels, hidden_channels, k = 3, s=1, p=1,)
...
class CSPNeXtLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.main_conv = Conv_v5(in_channels, mid_channels, 1)
        self.short_conv = Conv_v5(in_channels, mid_channels, 1)
        self.final_conv = Conv_v5(2 * mid_channels, out_channels, 1)
